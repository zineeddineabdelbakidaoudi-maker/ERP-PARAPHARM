...
fiscal_pages_content = '''# -*- coding: utf-8 -*-
import json
import csv
import tempfile
import os
...
class Annexe5Page(BaseFiscalPage):
    ...
    def export_pdf(self):
        ...
        pdf_path = os.path.join(tempfile.gettempdir(), f"annexe_5_{self.month_combo.currentText()}_{self.year_spin.value()}.pdf")
        period_str = f"{self.month_combo.currentText()} / {self.year_spin.value()}"
        try:
...
class DeclarationG50Page(BaseFiscalPage):
    ...
    def load_data(self):
        ...
        for k, lbl in [('ca_ht', 'Chiffre d\'Affaires Imposable (HT)'), ('tap', 'TAP (2%)'), 
            ...
            self.table.setItem(r, 1, QTableWidgetItem(self.g50_data[k]))
            if k == 'total':
                self.table.item(r, 0).setFont(QFont("Segoe UI", 10, QFont.Bold))
                self.table.item(r, 1).setFont(QFont("Segoe UI", 10, QFont.Bold))

    def export_pdf(self):
        pdf_path = os.path.join(tempfile.gettempdir(), f"g50_{self.month_combo.currentText()}_{self.year_spin.value()}.pdf")
        period_str = f"{self.month_combo.currentText()} / {self.year_spin.value()}"
        try:
...
class DemandesExonerationTVAPage(BaseFiscalPage):
    def __init__(self, user, parent=None):
        ...
        self.table = self.create_table(["Client", "NIF", "Motif", "N° Décision", "Date", "Montant HT", "Période"])
        self.main_layout.addWidget(self.table)
        self.load_data()
    ...
    def _add_exo(self):
        ...
        inp_periode = QLineEdit(datetime.now().strftime("%Y"))
        
        form.addRow("Client :", cb_client)
        form.addRow("Motif :", inp_motif)
        form.addRow("N° Décision :", inp_num)
        form.addRow("Date :", inp_date)
        form.addRow("Montant HT :", inp_montant)
        form.addRow("Période :", inp_periode)
    ...
    def load_data(self):
        ...
        for exo in self.db_session.query(ExonerationTVA).all():
            ...
            self.table.setItem(r, 5, QTableWidgetItem(format_money(exo.montant_ht)))
            self.table.setItem(r, 6, QTableWidgetItem(exo.periode))
...
class ChiffreAffaireFiscalPage(BaseFiscalPage):
    ...
    def load_data(self):
        ...
        for i in range(1, 13):
            ...
            self.table.setItem(r, 3, QTableWidgetItem(format_money(net)))
        ...
        ax.bar(months, ca_nets, color='#1565C0')
        ax.set_ylabel("CA Net HT (DA)")
        ax.set_title(f"Évolution CA {y}")
...
class RappelConvocationClientsPage(BaseFiscalPage):
    ...
    def load_data(self):
        ...
        for cid, info in client_debts.items():
            ...
            btn_print = QPushButton("📄 Imprimer Rappel")
