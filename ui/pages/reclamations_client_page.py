...
class ReclamationsClientPage(BaseDocumentPage):
    PAGE_TITLE = "Réclamations Client"
    STATUS_OPTIONS = ["Tous", "Ouvert", "En cours", "Résolu"]

    def _get_columns(self):
        return ["N°", "Date", "Client", "Motif", "Statut", "Actions"]

    def _load_data(self, search, status_filter):
        q = self.db_session.query(Reclamation).filter(Reclamation.client_id.isnot(None))
        ...
        data = []
        for r in recs:
            ent = r.client.name if r.client else ""
    ...
    def _on_add(self):
        ...
        dlg = ReclamationDialog(self.user, parent=self, is_client=True)
        if dlg.exec():
            self.refresh_data()
    ...
    def _on_edit(self, row_data):
        ...
        dlg = ReclamationDialog(self.user, parent=self, is_client=True, reclamation=rec)
        if dlg.exec():
            self.refresh_data()
