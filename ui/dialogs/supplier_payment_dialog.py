...
# -*- coding: utf-8 -*-
"""
ParaFarm ERP — Supplier Payment Dialog (Fiche Versements Fournisseur)
"""
...
class SupplierPaymentDialog(QDialog):
    """
    Fiche Versements Fournisseur
    Completely built to match the exact layout described in SECTION 2.
    """
    ...
    def __init__(self, user, parent=None):
        ...
        self.setWindowTitle("Versement Fournisseur")
    ...
    def _setup_ui(self):
        ...
        # Title
        title_lbl = QLabel("Versement Fournisseur")
        ...
        # Nom du Fournisseur with barcode & search
        form_grid.addWidget(QLabel("Nom du Fournisseur :"), 0, 0)
        supplier_selector_layout = QHBoxLayout()
        
        # Dropdown combobox with tooltip
        self.supplier_combo = SearchableComboBox()
        self.supplier_combo.setEditable(True)
        self.supplier_combo.setPlaceholderText("Rechercher ou sélectionner un fournisseur...")
        self.supplier_combo.setToolTip("Fiche Client (Ventes, versements, retours,...)")
        self.supplier_combo.currentIndexChanged.connect(self._on_supplier_selected)
        supplier_selector_layout.addWidget(self.supplier_combo, stretch=1)
        
        # Barcode button
        self.barcode_btn = QPushButton("📷")
        self.barcode_btn.setFixedSize(32, 32)
        self.barcode_btn.setStyleSheet("background-color: #78909C; color: white;")
        self.barcode_btn.setToolTip("Scanner Code-Barres Fournisseur")
        supplier_selector_layout.addWidget(self.barcode_btn)

        # Search button
        self.supplier_search_btn = QPushButton("🔍")
        self.supplier_search_btn.setFixedSize(32, 32)
        self.supplier_search_btn.setStyleSheet("background-color: #1E88E5; color: white;")
        self.supplier_search_btn.clicked.connect(self._open_supplier_picker)
        supplier_selector_layout.addWidget(self.supplier_search_btn)
        
        form_grid.addLayout(supplier_selector_layout, 0, 1)
        ...
        self.remboursement_checkbox = QCheckBox("Remboursement par le fournisseur (Après retour,...)")
        ...
        # Info button layout at bottom right
        info_layout = QHBoxLayout()
        info_layout.addStretch()
        self.info_btn = QPushButton("ℹ️")
        self.info_btn.setFixedSize(28, 28)
        self.info_btn.setToolTip("Informations complémentaires")
        self.info_btn.setStyleSheet("background-color: transparent; border: none; font-size: 16px; color: #555;")
        self.info_btn.clicked.connect(self._show_info)
        info_layout.addWidget(self.info_btn)
        body_panel.addLayout(info_layout)
    ...
    def _load_suppliers(self):
        ...
        self.suppliers_list = self.db_session.query(Supplier).filter(Supplier.is_deleted == 0, Supplier.is_active == 1).order_by(Supplier.name).all()
    ...
    def _on_supplier_selected(self, index):
        supp_id = self.supplier_combo.itemData(index)
        if supp_id is None:
            self.selected_supplier = None
            self._ancien_credit_val = 0.0
        ...
        self.selected_supplier = self.db_session.query(Supplier).filter(Supplier.id == supp_id).first()
        self._load_supplier_credit()

    def _load_supplier_credit(self):
    ...
    def _update_calculation(self):
        ...
        if self.remboursement_checkbox.isChecked():
            # Refund by supplier actually increases supplier credit
            nouveau = ancien + montant
        else:
            # Payment to supplier and discounts reduce our debt
            nouveau = ancien - montant - remise

        if nouveau < -0.01:
            # Overpaid — supplier owes us (avoir)
    ...
    def _open_supplier_picker(self):
        ...
        if picker.exec() and picker.selected_supplier:
            s = picker.selected_supplier
            for idx in range(self.supplier_combo.count()):
                if self.supplier_combo.itemData(idx) == s.id:
    ...
    def _show_info(self):
        QMessageBox.information(
            self, "Informations", 
            "Fiche Versements Fournisseur\n"
            "Ce module permet d'enregistrer des règlements envers vos fournisseurs et de suivre l'évolution de vos dettes."
        )
    ...
    def _on_validate(self):
        ...
        if method == "ESPECES":
            ...
            if not cash_repo.get_active_session():
                QMessageBox.warning(self, "Caisse Fermée", "La caisse doit être ouverte pour enregistrer un versement en espèces.")
                return
        ...
        try:
            ...
            for d in debts:
                ...
                self.debt_service.record_payment(
                    ...
                    reference=ref_num,
                    notes=self.observation_input.text().strip() or "Versement Fournisseur"
                )
            ...
            if remaining_to_deduct > 0 and not self.remboursement_checkbox.isChecked():
                ...
                new_debt = Debt(
                    ...
                    notes=self.observation_input.text().strip() or "Versement anticipé / Crédit Fournisseur"
                )
                self.db_session.add(new_debt)
                self.db_session.commit()

                # Record the cash register withdrawal if applicable
                if method == "ESPECES":
                    from app.repositories.finance_repository import CashRegisterRepository
                    cash_repo = CashRegisterRepository(self.db_session)
                    active = cash_repo.get_active_session()
                    if active:
                        active.total_withdrawals += remaining_to_deduct
                        active.expected_balance -= remaining_to_deduct
                        self.db_session.commit()

            QMessageBox.information(self, "Succès", "Le versement fournisseur a été enregistré avec succès.")
            self.accept()
