# -*- coding: utf-8 -*-
"""
ParaFarm ERP — View Purchase (BR) Dialog
"""
...
class ViewPurchaseDialog(QDialog):
    """
    Dialog displaying the full details of a Purchase (BR) document in a premium design.
    """

    def __init__(self, purchase_id_or_number, parent=None):
        super().__init__(parent)
        self.db_session = get_session()
        self.purchase = None
        
        # Load the purchase
        if isinstance(purchase_id_or_number, int):
            self.purchase = self.db_session.query(Purchase).filter(Purchase.id == purchase_id_or_number).first()
        else:
            self.purchase = self.db_session.query(Purchase).filter(Purchase.purchase_number == purchase_id_or_number).first()

        self.setWindowTitle(f"Détail du Bon de Réception: {self.purchase.purchase_number if self.purchase else 'Inconnu'}")
    ...
    def _setup_ui(self):
        ...
        # Title
        title_label = QLabel("BON DE RÉCEPTION")
        title_label.setStyleSheet("font-size: 18px; font-weight: bold; color: #E64A19;")
        header_grid.addWidget(title_label, 0, 0, 1, 2)

        # N° Bon
        header_grid.addWidget(QLabel("<b>N° BR / Facture:</b>"), 0, 2)
        self.lbl_num = QLabel("—")
        self.lbl_num.setStyleSheet("font-size: 14px; font-weight: bold; color: #D32F2F;")
        header_grid.addWidget(self.lbl_num, 0, 3)

        # Supplier Info
        header_grid.addWidget(QLabel("<b>Fournisseur:</b>"), 1, 0)
        self.lbl_supplier = QLabel("—")
        self.lbl_supplier.setStyleSheet("font-weight: bold; color: #2C3E50;")
        header_grid.addWidget(self.lbl_supplier, 1, 1)
        ...
        # Invoice Number
        header_grid.addWidget(QLabel("<b>N° Facture Fournisseur:</b>"), 2, 0)
        self.lbl_inv_num = QLabel("—")
        header_grid.addWidget(self.lbl_inv_num, 2, 1)
        ...
        # ── LINE ITEMS GRID ─────────────────────────────────────
        cols = ["N° Ordre", "Référence", "Désignation", "Qté Reçue", "Prix Achat U", "Total"]
        ...
        totals_grid.addWidget(QLabel("<b>TOTAL NET:</b>"), 2, 0)
    ...
    def _load_data(self):
        ...
        self.lbl_num.setText(self.purchase.purchase_number)
        self.lbl_supplier.setText(self.purchase.supplier.name if self.purchase.supplier else "—")
        self.lbl_date.setText(self.purchase.purchase_date.split(' ')[0] if self.purchase.purchase_date else "—")
        self.lbl_inv_num.setText(self.purchase.invoice_number or "—")
        self.lbl_status.setText(self.purchase.status or "VALIDE")
        ...
        for i, item in enumerate(self.purchase.items):
            ...
            price_item = QTableWidgetItem(f"{item.unit_cost:,.2f}".replace(",", " "))
        ...
        # Set totals
        self.lbl_tot_brut.setText(f"{self.purchase.total_amount:,.2f} DA".replace(",", " "))
        self.lbl_remise.setText(f"{self.purchase.discount_amount:,.2f} DA".replace(",", " "))
        net = self.purchase.total_amount - self.purchase.discount_amount
