...
# Replace the broken method completely using a regex
broken_method_pattern = r'    def _show_delivery_history\(self\):.*?dlg\.exec\(\)'
if not re.search(broken_method_pattern, text, flags=re.DOTALL):
    print("Could not find the broken method!")
else:
    new_method = """    def _show_delivery_history(self):
        ...
        for i, d in enumerate(deliveries):
            ...
            copy_btn.clicked.connect(lambda checked, delivery=d: (self._apply_copy_from_delivery(delivery), dlg.accept()))
    ...
    text = re.sub(broken_method_pattern, new_method, text, flags=re.DOTALL)
    
    with open('ui/pages/deliveries_page.py', 'w', encoding='utf-8') as f:
        f.write(text)
    print("Fixed deliveries_page.py")
